...
public class DecimalKeyboard extends BaseKeyboard {
    ...
    public DecimalKeyboard(Context context, KeyboardParams params, BaseKey.KeyListener
            keyListener) {
    ...
    protected View initKeyBoard(DisplayMetrics dm) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < column; j++) {
                if (i < 3) {
                    lp = new LayoutParams(avg, itemHeight, 1);
                } else {
                    lp = new LayoutParams(avg, itemHeight * 2 + 2, 1);
                }
