 ...
 */
public class DeleteKey extends BaseKey implements View.OnClickListener {

    Bitmap bitmap;

    public DeleteKey(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    // 构造函数，需要按键参数和回调接口
    public DeleteKey(Context context, KeyParams params, KeyListener listener) {
        this(context, null, params, listener);
    }

    // 构造函数，需要按键参数和回调接口
    public DeleteKey(Context context, AttributeSet attrs, KeyParams params, KeyListener listener) {
    ...
    public void setLayoutParams(ViewGroup.LayoutParams params) {
        ...
        if (Utils.isEmpty(mParams.mCharacter) && Utils.isNotEmpty(mParams.imageSrc) && bitmap != null) {
            ...
            if (params.width < params.height) {
                lp.width = (int)(params.width / 3.3);
                lp.height = (int)(lp.width * (bitmap.getHeight() / (bitmap.getWidth() * 1.0)));
            } else {
                lp.height = (int)(params.height / 3.3);
                lp.width = (int)(lp.height * (bitmap.getWidth() / (bitmap.getHeight() * 1.0)));
            }
